...
Developer reference for workspace boundaries, translation, CLI examples and verification. For the user-facing introduction, see [DustRoute](../README.md). For supported MCP workflows, see the [public feature guide](mcp-public-features.md).
...
Observed two-sided 3×3 piston-door shapes have a separate read-only
recognition contract in
[`docs/observed-3x3-piston-door.md`](observed-3x3-piston-door.md).  It
reports mechanical geometry, panel state evidence, and bounded control
connectivity without turning an existing snapshot into a runnable scenario or
publishing a dedicated MCP operation.
...
The logical/physical boundary supports the current MCP observation, revision,
planning and verified execution workflows. See the public feature guide for
the supported scope of each operation.

## Development

Minecraft-vs-simulator physical trace comparison is documented in
[`docs/physics-differential-testing.md`](physics-differential-testing.md).
...
cat scenario.json | cargo run -p dustroute-cli -- run-piston-door - open --translate 37,11,29 --diagnostic
...
for tool use, and [MCP setup](../crates/dustroute-mcp/SETUP.md)
...
[`docs/vanilla-instrumentation.md`](vanilla-instrumentation.md) for the
...
General real-world placement candidates must pass the shared immutable `ValidatedWorld` boundary.
The fixed 1x2 piston preset uses a separate immutable, exact-contract proof;
see [the pinned contract](piston-door-mcp-v1.md).
Compiler results, full-world Java export, placement planning, and live MCP
forward mutations use this validation. Editing a world discards its validation;
persisted plans are revalidated against a fresh snapshot before application.
See [`docs/world-validation-boundary.md`](world-validation-boundary.md).

`run-piston-door` rejects the current unbuildable fanout with structured
`world_validation_failed` issues before execution. Explicit `--diagnostic`
replays its synthetic model and returns `status = "diagnostic_complete"` plus
the failed placement assessment; this is not a deployable door result.

The diagnostic command loads the versioned `PistonDoorScenario` contract and uses
