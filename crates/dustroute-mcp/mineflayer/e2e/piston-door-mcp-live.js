'use strict'
// Private Java 1.21.11 only. Builds the verified fixture, then uses actual MCP tools.
...
async function run () {
  ...
  for (let trial = 0; trial < 3; trial++) {
    ...
    for (const b of fixture.initial.blocks) {
      ...
      await command(`/setblock ${coords(b.pos)} ${b.name}${props ? `[${props}]` : ''}`)
    }
    await sleep(1500)
    const outcomes = []
    ...
    console.log(`MCP door trial ${trial + 1}: matched`)
  }
  const noop = await plan('closed'); await mcp.callTool('show_operation', { operation_id: noop.operation_id })
  report.noop = await mcp.callTool('invoke_operation', { operation_id: noop.operation_id, confirm: true })
  assert.equal(report.noop.changed, false)
  const stale = await plan('open'); await mcp.callTool('show_operation', { operation_id: stale.operation_id })
  await command(`/setblock ${coords(low)} stone`)
  report.stale = await mcp.callToolRaw('invoke_operation', { operation_id: stale.operation_id, confirm: true })
  assert.equal(report.stale.ok, false)
  assert.equal(report.stale.observation.state, 'configuration_mismatch')
  assert.equal(bot.blockAt(abs(fixture.input)).getProperties().powered, true)
...
run().catch(error => { report.status = 'failed'; report.error = error.stack; report.mcp_stderr = mcp.stderr; process.exitCode = 1 }).finally(async () => {
  ...
  fs.writeFileSync(path.join(root, '.local/e2e-artifacts/piston-door-mcp-latest.json'), JSON.stringify(report, null, 2) + '\n')
